  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (promoCode) {
      const promo = await Promo.findOne({ code: promoCode })
        .select(
          "code percentage count minimumOrderValue maxDiscountAmount startsAt endsAt",
        )
        .lean<PromoRow | null>();
      ...
      if (
        foundPromo.maxDiscountAmount &&
        discountAmount > foundPromo.maxDiscountAmount
      ) {
        discountAmount = foundPromo.maxDiscountAmount;
      }
    }
    ...
    console.log(`[Checkout] Creating Razorpay order for amount: ${totalAmount}`);
