...
function parsePromoPayload(req: Request) {
  ...
  if (Number.isNaN(minimumOrderValue) || minimumOrderValue < 0) {
    throw new AppError(400, "Minimum order value must be atleast 0 or more");
  }
  ...
  if (
    ...
    throw new AppError(400, "Max discount amount must be 0 or more");
  }
...
adminPromoRouter.patch(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    foundPromo.maxDiscountAmount = payload.maxDiscountAmount;
