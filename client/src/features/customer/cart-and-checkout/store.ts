...
const defaultUiState = {
  ...
  availablePromos: [] as { code: string; percentage: number; minimumOrderValue: number }[],
  points: 0,
  promoLoading: false,
  ...
  create<CustomerCartAndCheckoutStore>((set, get) => ({
    ...
    loadCart: async (isSignedIn) => {
      try {
        ...
        if (isSignedIn) {
          ...
          const [response, promos] = await Promise.all([
            getCheckoutData(),
    ...
    applyPromo: async () => {
      ...
      try {
        ...
        set({ appliedPromo: null, promoLoading: false });
        toast.error(e?.message || "Unable to apply promo");
      }
    },
    clear: () =>
      set({
    ...
    startRazorpayCheckout: async ({ isSignedIn, name, email, onSuccess }) => {
      ...
      try {
        ...
        console.error("[CheckoutStore] Error starting checkout:", error);
        set({ checkoutLoading: false });
        toast.error(
